  ...
  // we want to test the function that is calling the external service.
    ...
    describe('and there are records in the sample data', () => {
      ...
      it('returns a list of records', done => {
        client.getCustomer((error,response)=>{
    ...
    describe('and there is an record in the data with that ID', () => {
      before(() => provider.addInteraction({
        state: 'Has a record with ID '+expectedRecords[0]['customerId'],
        uponReceiving: 'a request for a record with ID '+expectedRecords[0]['customerId'],
        withRequest: {
          method: 'GET',
          path: BASE_PATH+'/'+expectedRecords[0]['customerId']
        },
        willRespondWith: {
          status: 200,
          headers: {
      ...
      it('returns the record', done => {
        client.getCustomerById(expectedRecords[0]['customerId'],(error,response)=>{
          var parsedData = JSON.parse(response.body);
          expect(parsedData).to.have.deep.property('customerId', expectedRecords[0]['customerId']);
    ...
    describe('and there no records in the database', () => {
      ...
      it('returns a 404', done => {
        client.getCustomerById('NonExisting',(error,response)=>{
      ...
      describe('and there is a record in the data with that ID', () => {
        before(() => provider.addInteraction({
          state: 'Has a record with ID '+expectedRecords[0]['customerId'],
          uponReceiving: 'a request to update a record with ID '+expectedRecords[0]['customerId'],
          withRequest: {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json'
        ...
        it('returns the updated record', done => {
          client.putCustomer(expectedRecords[0]['customerId'], expectedRecords[0], (error,response)=>{
            var parsedData = JSON.parse(response.body);
            expect(parsedData).to.have.deep.property('customerId', expectedRecords[0]['customerId']);
            done();
          })
      ...
      describe('and there no records in the database with that ID', () => {
        ...
        it('returns a 404', done => {
          client.putCustomer('NonExisting', expectedRecords[0], (error,response)=>{
            expect(response.statusCode).to.be.eql(404);
            done();
          })
        })
      ...
      describe('and the record abides by the required schema', () => {
        ...
        it('returns the newly added record', done => {
          client.postCustomer(expectedRecords[0],(error,response)=>{
            var parsedData = JSON.parse(response.body);
            expect(parsedData).to.have.deep.property('customerId', expectedRecords[0]['customerId']);
            done();
          });
      ...
      describe('and the record is invalid with respect to the schema', () => {
        ...
        it('returns 400 error', done => {
          client.postCustomer({ 'dummyKey': 'dummyValue' },(error,response)=>{
      ...
      delete semiRecord['customerId'];
      describe('and there is an record in the data with that ID', () => {
        before(() => provider.addInteraction({
          state: 'Has a record with ID '+expectedRecords[0]['customerId'],
          uponReceiving: 'a request to semi-update a record with ID '+expectedRecords[0]['customerId'],
          withRequest: {
            method: 'PATCH',
            headers: {
              'Content-Type': 'application/json'
        ...
        it('returns the updated record', done => {
          client.patchCustomer(expectedRecords[0]['customerId'], semiRecord, (error,response)=>{
            var parsedData = JSON.parse(response.body);
            expect(parsedData).to.have.deep.property('customerId', expectedRecords[0]['customerId']);
            done();
          })
      ...
      describe('and there no records in the database with that ID', () => {
        ...
        it('returns a 404', done => {
          client.patchCustomer('NonExisting', semiRecord, (error,response)=>{
            expect(response.statusCode).to.be.eql(404);
            done();
          })
        })
      ...
      describe('and there is a record in the data with that ID', () => {
        before(() => provider.addInteraction({
          state: 'Has a record with ID '+expectedRecords[0]['customerId'],
          uponReceiving: 'a request to delete a record with ID '+expectedRecords[0]['customerId'],
          withRequest: {
            method: 'DELETE',
            path: BASE_PATH+'/'+expectedRecords[0]['customerId']
          },
        ...
        it('returns 204', done => {
          client.deleteCustomer(expectedRecords[0]['customerId'], (error,response)=>{
      ...
      describe('and there no records in the database with that ID', () => {
        ...
        it('returns a 404', done => {
          client.deleteCustomer('NonExisting', (error,response)=>{
            expect(response.statusCode).to.be.eql(404);
            done();
          })
        })
